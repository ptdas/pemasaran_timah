frappe.pages['sales-analytics-curr'].on_page_load = function(wrapper) {
	...
	new erpnext.SalesAnalyticswithCurrency(wrapper);

	frappe.breadcrumbs.add("Selling")
}

erpnext.SalesAnalyticswithCurrency = frappe.views.TreeGridReport.extend({
	init: function(wrapper) {
		this._super({
			...
			doctypes: ["Item", "Item Group", "Customer", "Customer Group", "Company", "Territory",
				...
				"Sales Order[Sales Analytics with Currency]", "Sales Order Item[Sales Analytics with Currency]",
				"Delivery Note[Sales Analytics with Currency]", "Delivery Note Item[Sales Analytics with Currency]"],
			tree_grid: { show: true }
		});
	...
	filters: [
		...
		{fieldtype:"Select", label: __("Currency"), fieldname: "currency",
				options:[{label: __("USD"), value: "USD"}, {label: __("IDR"), value: "IDR"}]}
	],
	setup_filters: function() {
		var me = this;
		this._super();

		this.trigger_refresh_on_change(["value_or_qty", "tree_type", "based_on", "company", 'currency']);
		...
		$.each(this.tl[this.based_on], function(i, tl) {
			if (me.is_default('company') ? true : tl.company === me.company) {
				...
				if (posting_date >= from_date && posting_date <= to_date) {
					...
					if(item){
						item[me.column_map[tl.posting_date].field] += (is_val ? (is_idr ? (tl.currency == 'IDR' ? tl.net_amount : 0) : tl.base_net_amount) : tl.qty);
					}
				}
			}
		});
