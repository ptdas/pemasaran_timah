...
def execute(filters=None):
	...
	for row in data:
		...
		check_booking = frappe.db.sql("""SELECT SUM(qty)
					FROM `tabBooking Batch Item` bbi JOIN `tabBooking Form` tb ON tb.name = bbi.parent 
					WHERE bbi.`item_code` = "{}" AND tb.`docstatus` = 1 AND tb.`booking_status` = "Booked" 
					AND warehouse = "{}" """.format(row[0],row[5]))
		...
		temp.append(row[13])
		if check_booking:
			temp.append(flt(check_booking[0][0]))
			temp.append(row[13]-flt(check_booking[0][0]))
		...
		data_akhir.append(temp)


	return columns, data_akhir
